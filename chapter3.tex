...
space into pieces. In the previous chapter we discussed a way of computing homology from a 
...
In this chapter we design and implement a framework for the parallel computation of field homology using the \mv principle. Instead of directly
treating the \mv spectral sequence, we discuss the algorithm in terms of a topological analogue, the \mvb{} is used to discuss the algorithm. 
The \mvb{} is used by Zomorodian and Carlsson to compute localized homology~\cite{zc-lh-08}.  The procedures discussed in this chapter 
...
\end{theorem}
\begin{proof}
Define the map $\pi: \K^\C \rightarrow \K$ which projects an element in the blowup complex to an element in the base complex. One may triangulate $\K^\C$ to produce a larger, homotopic simplicial complex. It then follows by functorality that $\pi^*$ is an induced surjective linear map on homology.  To find an explicit homotopy inverse, one takes a \emph{partition of unity} $\{\phi_\alpha\}$ subordinate to $\C$ to produce a map: $\phi: \K \rightarrow \K^\C$. it is not difficult to see that $\phi$ and $\pi$ are homotopy equivalent.
\end{proof}
