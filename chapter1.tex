...
%\While{$\col{R}{i} \neq 0$ and $\Exists j < i$ with $\Youngest[\col{R}{i}]= \Youngest[\col{R}{j}]$}
%    \State $\col{R}{i} = \col{R}{i} - \alpha^{-1}_{ij}\col{R}{j}$
%    \State $U[j,i] = \alpha_{ij}$
%\EndWhile
%\EndFor
%\end{algorithm}
%\end{algorithmic}
...
%The first key ingredient is the observation that $\partial_{k-1}(\partial_k(\sigma)) = 0$. Suppose we order $\partial_k(\sigma) = \sum_{i=0}^k \alpha_i c_i$. Then, the identity $\partial^2=0$ implies that we may write $\partial(c_k)$= -\sum_{i=0}^{k-1} \partial(c_i)$. If we order the columns of $\partial$ so that the column corresponding  to $\partial_(c_k)$ appears to the right of the remaining boundary elements, then we have concluded that $\partial(c_k)$ is linearly dependent on the previous columns, in other words, the persistence algorithm will reduce this column to be zero! We may save $O(m)$ column additions by simply immediately zeroing out this column. 
%
%On the other hand, we have written $\partial_k(\sigma)$ in terms of the elements $c_i$. This suggests that if we order the rows of $\partial_k$ in the same order as the columns of $\partial_{k-1}$ that whenever we find a pivot in row $j$, that column $j$ may be eliminated.
