...
\begin{tikzpicture}[scale=.5]
	...
	\begin{pgfonlayer}{ball}
        \fill[gray!50,radius= .6 cm] (\x,\y) circle{};
        \end{pgfonlayer}{ball}
	\node[draw, circle, scale=.25, fill=white](\p) at (\x, \y) {$\p$};
    } 
 \end{tikzpicture}
 \begin{tikzpicture}[scale=.5]
	\node[draw=none, fill=white] at (0, -5.5) {$\check{C}_{.6}$};
...
\input{cell_5_nerve}
 \end{tikzpicture}
\caption{The Cech complex at $\epsilon = .6$ for a set of points on the annulus}
\end{figure}
...
\end{definition}
These two simplicial complexes are related.
\begin{theorem}
