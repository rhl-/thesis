...
A \emph{topological space} on a set $X$ is the pair $(X,T)$ where $T$ is a subcollection of $P(X)$, containing $\emptyset$ and $X$, and is further closed under countable unions, and finite intersections. 
\end{definition}
Elements of $T$ are referred to as open sets. The complement of an open set is a closed set. Some examples of topological spaces are, the $D^n$ the unit disk in $R^n$, $S^n$ the unit sphere in $R^{n+1}$. and $\partial D^n = S^{n-1}$ the boundary of the $n$-disk.  
...
\end{theorem}
\begin{proof}
See~\cite{paper-from-the-90s}
\end{proof}
Importantly when we have a finitely generated module over a principal ideal domain, we have a structure theorem. We state it in slightly less generality:
\begin{theorem}{Structure Theorem}
Any finitely generated $k[x]$-module can be decomposed into indecomposables, which are of a specific form: 
\end{theorem}
\begin{proof}
See~\cite{}.
\end{proof}
